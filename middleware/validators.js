...
const checkWavFile = (req, res, next) => {
  if (!req.file)
    return res
      ...
      .json({ error: "Aucun fichier téléchargé" });
  ...
  if (header.toString("utf-8") !== "RIFF")
    return res
      .status(HTTP_STATUS.BAD_REQUEST)
      .json({ error: "Le fichier n'est pas au format wav" });
...
const checkArg = (req, res, next) => {
  if (!req.body.language)
    return res
      .status(HTTP_STATUS.BAD_REQUEST)
      .json({ error: "Il manque la langue de la voice" });
  if (!isStringEmpty(req.body.language))
    return res
      .status(HTTP_STATUS.BAD_REQUEST)
      .json({ error: '"language" n\'est pas un string ou est vide' });
  next();
};
